...
# array=( 0  2   4   6   9  11  12  25  29  36  46  50  57
#   59  68  70  75  81  83  84  85  86  87  97  98  99
array=(  103 104 105 106 107 111 113 114 115 116 117 118 119
...
do
  ...
  if [ -d "$folder_name" ]; then
    ...
    /usr/bin/time -v -o fed_prove_perf.txt ezkl prove --proof-type=for-aggr -W witness.json -M network.ezkl --proof-path site${i}.pf --srs-path=23.srs  --pk-path=pk.key
    rm *.srs
  fi
done >> logs/fed_prove_perf.txt